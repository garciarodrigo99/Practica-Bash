...
opt_count=0					# Opcion que nos dice si muestra todos los procesos que superen el parametro
...
imprimir()
{
if [[ $opt_count -eq 0 ]];then	# [Caso no -count]
	echo "$1"	""$(id -gn "$2")"	"$(ps --no-header -u "$2" | wc -l)""
else							# [Caso -count]
	echo "$1"	""$(id -gn "$2")"	"$(ps --no-header -u "$2" | wc -l)"	"$3""
fi
}
...
PS	Numero de procesos totales del usuario, independientemente del 
	parametro tiempo
PS_TIME	Numero total de procesos que superan el parametro tiempo
_EOF_
...
do
    ...
    if [[ "$proc_time" -gt "$time_select" ]]; then										# Para comparar el tiempo del proceso con el tiempo establecido. 	#[Caso ps cumple con parametro tiempo]
		if [[ ! $usuario ]]; then														# Si esta vacio, relleno con el primer usuario. 					# [Caso variable usuario vacio]
			...
			line_aux=$line
			aux_user=$usuario
			ps_counter=1																# Inicializo la cuenta total de procesos que cumplen parametro tiempo
    		...
    		if [[ "$usuario" != $(echo $line | tr -s ' ' '_' | cut -d_ -f1) ]]; then	# Comprobar que el usuario distinto para asignar el nuevo usuario.	#[Caso usuario distinto al anterior]
				usuario=$(echo $line | tr -s ' ' '_' | cut -d_ -f1)						# Relleno nuevo usuario
				subfuncion "$line_aux" $aux_user $ps_counter
				line_aux=$line
				aux_user=$usuario
				ps_counter=1															# Pongo de nuevo cuenta a 1
			else
				line_aux=$line
				aux_user=$usuario
...
if [[ $opt_count -eq 0 ]];then
	echo "${TEXT_ULINE}$(ps -eo user:20,uid,pid,times | head -n 1)"" GUSER	PS${TEXT_RESET}"			# Mostrar encabezado sin -count
else
	echo "${TEXT_ULINE}$(ps -eo user:20,uid,pid,times | head -n 1)"" GUSER	PS	PS_TIME${TEXT_RESET}"	# Mostrar encabezado sin -count
fi
